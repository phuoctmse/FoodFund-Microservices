...
import { CommentModel, CreateCommentInput, UpdateCommentInput, CommentWithRelations } from '../comment.model';
...
export class CommentRepository extends BaseRepository<CommentModel> {
  ...
  async updateComment(id: string, data: UpdateCommentInput): Promise<CommentModel> {
  ...
  async getRecentComments(campaignId: string, limit: number = 5): Promise<CommentWithRelations[]> {
    return this.getModel().findMany({
      where: { campaignId },
      include: {
        author: {
