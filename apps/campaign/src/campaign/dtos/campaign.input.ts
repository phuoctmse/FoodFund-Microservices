...
export class CreateCampaignInput {
    ...
    @Field(() => String, { description: "Target amount as string (BigInt compatible)" })
...
export class UpdateCampaignInput {
    ...
    @Field(() => String, { nullable: true, description: "Campaign description" })
    @IsOptional()
    @IsString({ message: "Description must be a string" })
    @IsNotEmpty({ message: "Description cannot be empty if provided" })
    description?: string

    @Field(() => String, { nullable: true, description: "Campaign cover image URL" })
    @IsOptional()
    @IsString({ message: "Cover image URL must be a string" })
    coverImage?: string
    ...
    @Field(() => String, { nullable: true, description: "Target amount as string" })
    @IsOptional()
    @IsString({ message: "Target amount must be a string" })
    targetAmount?: string
...
export class CampaignFilterInput {
    @Field(() => [CampaignStatus], { nullable: true, description: "Filter by campaign status" })
    @IsOptional()
    @IsEnum(CampaignStatus, { each: true, message: "Each status must be a valid CampaignStatus" })
    status?: CampaignStatus[]

    @Field(() => String, { nullable: true, description: "Filter by creator ID" })
    @IsOptional()
...
registerEnumType(CampaignSortOrder, {
    ...
    valuesMap: {
        ACTIVE_FIRST: {
            description: "Sort by campaign status (active campaigns first), then by creation date"
        },
        NEWEST_FIRST: {
